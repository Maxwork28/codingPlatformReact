...
const QuestionSubmission = () => {
  ...
  const examIdFromState = location.state?.examId || searchParams.get('examId');
  const examAttemptIdFromState = location.state?.examAttemptId || searchParams.get('examAttemptId');
  const examContext = examIdFromState ? { examId: examIdFromState, examAttemptId: examAttemptIdFromState } : undefined;
  const { question, submission, runResults, status, error } = useSelector(
  ...
  const questionSectionId = location.state?.questionSectionId || null;
  const [questionTimerSeconds, setQuestionTimerSeconds] = useState(
    location.state?.questionTimeRemaining ?? null
  );
  const [sectionTimerSeconds, setSectionTimerSeconds] = useState(
    location.state?.sectionTimeRemaining ?? null
  );
  const [totalTimerMs, setTotalTimerMs] = useState(null);
  ...
  const classId =
    ...
    (question?.classes?.length > 0 ? question.classes[0].classId : null);
  ...
  useEffect(() => {
    if (questionId) {
      console.log(
        "[QuestionSubmission] Fetching question with ID:",
        questionId
      );
  ...
  useEffect(() => {
    if (!examContext?.examId || !examContext.examAttemptId) return;
    let mounted = true;
    const loadAttempt = async () => {
      try {
        const res = await getExamAttempt(examContext.examId);
        if (!mounted) return;
        const attempt = res.data.attempt;
        setAttemptData(attempt);
        if (attempt?.endsAt) {
          setTotalTimerMs(Math.max(new Date(attempt.endsAt).getTime() - Date.now(), 0));
        }
        if (questionSectionId) {
          const sectionEntry = attempt.sectionTimers?.find((timer) => timer.sectionId === questionSectionId);
          if (sectionEntry) {
            setSectionTimerSeconds(sectionEntry.remainingSeconds ?? null);
          }
        } else if (attempt.currentSectionId) {
          const sectionEntry = attempt.sectionTimers?.find((timer) => timer.sectionId === attempt.currentSectionId);
          if (sectionEntry) {
            setSectionTimerSeconds(sectionEntry.remainingSeconds ?? null);
          }
        }
        const questionEntry = attempt.questionTimers?.find(
          (timer) => String(timer.questionId) === String(questionId)
        );
        if (questionEntry) {
          setQuestionTimerSeconds(questionEntry.remainingSeconds ?? null);
          if (questionEntry.remainingSeconds === 0) {
            setQuestionLocked(true);
          }
        } else if (location.state?.questionTimeRemaining != null) {
          const remaining = location.state.questionTimeRemaining;
          setQuestionTimerSeconds(remaining);
          if (remaining === 0) setQuestionLocked(true);
        }
      } catch (err) {
        console.error('[QuestionSubmission] loadAttempt error', err);
      }
    };
    loadAttempt();
    return () => {
  ...
  }, [examContext, questionId, questionSectionId, location.state]);

  useEffect(() => {
    if (!examContext?.examId || questionLocked) return;
    timerIntervalRef.current && clearInterval(timerIntervalRef.current);
    timerIntervalRef.current = window.setInterval(() => {
      setTotalTimerMs((prev) => (prev === null ? prev : Math.max(prev - 1000, 0)));
      setSectionTimerSeconds((prev) => (prev === null ? prev : Math.max(prev - 1, 0)));
  ...
  useEffect(() => {
    if (question && classId) {
      ...
      socket.on(
        "questionPublished",
        ({ questionId: updatedId, isPublished }) => {
          if (updatedId === questionId) {
            const classData = question.classes.find(
              (cls) => cls.classId === classId
            );
            const newActiveState = classData
              ? isPublished && !classData.isDisabled
              : false;
            console.log(
              "[QuestionSubmission] Updating isQuestionActive:",
              newActiveState
            );
      ...
      socket.on("questionDisabled", ({ questionId: updatedId, isDisabled }) => {
        if (updatedId === questionId) {
          ...
          const newActiveState = classData
            ? classData.isPublished && !isDisabled
            : false;
          console.log(
            "[QuestionSubmission] Updating isQuestionActive:",
            newActiveState
      ...
      socket.on(
        "codeRun",
        ({ submissionId, isCorrect, passedTestCases, totalTestCases }) => {
          console.log("[QuestionSubmission] codeRun event:", {
            submissionId,
            isCorrect,
            passedTestCases,
            totalTestCases,
          ...
          setStatusMessage(
            `Run completed: ${passedTestCases}/${totalTestCases} test cases passed`
          );
        }
      );
      ...
      socket.on(
        "customInputRun",
        ({ submissionId, customInput, expectedOutput }) => {
          console.log("[QuestionSubmission] customInputRun event:", {
            submissionId,
            customInput,
            expectedOutput,
          });
      ...
      socket.on(
        "submissionUpdate",
        ({ submissionId, isCorrect, passedTestCases, totalTestCases }) => {
          console.log("[QuestionSubmission] submissionUpdate event:", {
            submissionId,
            isCorrect,
            passedTestCases,
            totalTestCases,
          ...
          setStatusMessage(
            `Submission completed: ${passedTestCases}/${totalTestCases} test cases passed`
          );
        }
      );
  ...
  useEffect(() => {
    if (question && classId) {
      ...
      if (
        ...
        const starterCode = question.starterCode?.find(
          (sc) => sc.language === selectedLanguage
        );
        console.log(
          "[QuestionSubmission] Setting starter code for",
          selectedLanguage,
          ":",
          starterCode?.code
        );
  ...
  useEffect(() => {
    if (!examContext?.examId || !examContext.examAttemptId) return;
    if (totalTimerMs === null) return;
    if (totalTimerMs > 0) return;
    if (autoSubmitTriggeredRef.current) return;
    autoSubmitTriggeredRef.current = true;
    autoSubmitExam(examContext.examId, examContext.examAttemptId).catch((err) => {
      console.error('[QuestionSubmission] autoSubmitExam error', err);
    });
  }, [examContext, totalTimerMs]);

  useEffect(() => {
    if (!examContext?.examId || !examContext.examAttemptId) return;
    if (questionTimerSeconds === null || questionId == null) return;
    if (questionTimerSeconds < 0) return;
    if (questionTimerSeconds === 0 && !questionLocked) {
      setQuestionLocked(true);
      setStatusMessage("Time for this question has expired");
    }
    if (questionTimerSeconds === 0 || questionTimerSeconds % 15 === 0) {
      updateQuestionTimer(examContext.examId, {
        attemptId: examContext.examAttemptId,
        questionId,
        remainingSeconds: Math.max(questionTimerSeconds, 0),
        completed: questionTimerSeconds === 0 || questionLocked
      }).catch((err) => console.error('[QuestionSubmission] updateQuestionTimer error', err));
    }
  }, [examContext, questionTimerSeconds, questionId, questionLocked]);

  useEffect(() => {
    if (!examContext?.examId || !examContext.examAttemptId) return;
    if (sectionTimerSeconds === null || !questionSectionId) return;
    if (sectionTimerSeconds < 0) return;
    if (sectionTimerSeconds === 0 || sectionTimerSeconds % 15 === 0) {
      updateSectionTimer(examContext.examId, {
        attemptId: examContext.examAttemptId,
        sectionId: questionSectionId,
        remainingSeconds: Math.max(sectionTimerSeconds, 0),
        currentQuestionId: questionId
      }).catch((err) => console.error('[QuestionSubmission] updateSectionTimer error', err));
    }
  }, [examContext, sectionTimerSeconds, questionSectionId, questionId]);

  useEffect(() => {
    return () => {
      timerIntervalRef.current && clearInterval(timerIntervalRef.current);
      if (examContext?.examId && examContext.examAttemptId) {
        if (questionSectionId && sectionTimerSeconds !== null) {
          updateSectionTimer(examContext.examId, {
            attemptId: examContext.examAttemptId,
            sectionId: questionSectionId,
            remainingSeconds: Math.max(sectionTimerSeconds, 0)
          }).catch((err) => console.error('[QuestionSubmission] cleanup updateSectionTimer error', err));
        }
        if (questionTimerSeconds !== null) {
          updateQuestionTimer(examContext.examId, {
            ...
            remainingSeconds: Math.max(questionTimerSeconds, 0),
            completed: questionTimerSeconds === 0 || questionLocked
          }).catch((err) => console.error('[QuestionSubmission] cleanup updateQuestionTimer error', err));
        }
      }
    };
  }, [examContext, questionTimerSeconds, sectionTimerSeconds, questionId, questionSectionId, questionLocked]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (examContext && questionLocked) {
      setStatusMessage("Cannot submit: time for this question has expired");
      return;
    }
    if (!isQuestionActive) {
      console.log(
        "[QuestionSubmission] Submission blocked: Question is inactive"
      );
    ...
    if (!classId) {
      console.log(
        "[QuestionSubmission] Submission blocked: classId is undefined"
      );
    ...
    if (
      ...
      console.log(
        "[QuestionSubmission] Submission blocked: Invalid or undefined language",
        selectedLanguage
      );
    ...
    if (question.type === "fillInTheBlanks" && !answer.trim()) {
      console.log(
        "[QuestionSubmission] Submission blocked: Empty answer for fill-in-the-blanks"
      );
    ...
    if (question.type === "fillInTheBlanksCoding" && !answer.trim()) {
      console.log(
        "[QuestionSubmission] Submission blocked: Empty answer for fill-in-the-blanks coding"
      );
      setStatusMessage("Cannot submit: Answer cannot be empty");
      return;
    }
    if (
      ...
      console.log(
        "[QuestionSubmission] Submission blocked: No option selected for MCQ"
      );
    ...
    try {
      ...
      const result = await dispatch(
        submitQuestionAnswer(submissionData)
      ).unwrap();
      console.log(
        "[QuestionSubmission] Submission result:",
      ...
      const normalizedSubmission = {
        ...result.submission,
        passedTestCases:
          result.passedTestCases ?? result.submission.passedTestCases ?? 0,
        totalTestCases:
          result.totalTestCases ?? result.submission.totalTestCases ?? 0,
      ...
      setStatusMessage(
        `Submission completed: ${normalizedSubmission.passedTestCases}/${normalizedSubmission.totalTestCases} test cases passed`
      );
    } catch (err) {
  ...
  const handleRunCode = async () => {
    if (examContext && questionLocked) {
      setStatusMessage("Cannot run code: time for this question has expired");
      return;
    }
    if (!isQuestionActive) {
      console.log(
        "[QuestionSubmission] Code run blocked: Question is inactive"
      );
    ...
    if (
      ...
      console.log(
        "[QuestionSubmission] Code run blocked: Not a coding question"
      );
    ...
    if (!classId) {
      console.log(
        "[QuestionSubmission] Code run blocked: classId is undefined"
      );
      setStatusMessage(
        "Cannot run code: No class associated with this question"
      );
      return;
    }
    if (!selectedLanguage || !question.languages.includes(selectedLanguage)) {
      console.log(
        "[QuestionSubmission] Code run blocked: Invalid or undefined language",
        selectedLanguage
      );
    ...
    setApiResponse(null);
    try {
      console.log("[QuestionSubmission] Running code:", {
        questionId,
        answer,
        classId,
      ...
      setStatusMessage(
        `Run completed: ${result.submission.passedTestCases}/${result.submission.totalTestCases} test cases passed`
      );
    } catch (err) {
  ...
  const handleRunCodeWithCustomInput = async () => {
    ...
    if (!isQuestionActive) {
      console.log(
        "[QuestionSubmission] Custom input run blocked: Question is inactive"
      );
      setStatusMessage("Cannot run code: Question is inactive");
      return;
    }
    if (
      ...
      console.log(
        "[QuestionSubmission] Custom input run blocked: Not a coding question"
      );
      setStatusMessage("Run is only available for coding questions");
      return;
    }
    if (!classId) {
      console.log(
        "[QuestionSubmission] Custom input run blocked: classId is undefined"
      );
      setStatusMessage(
        "Cannot run code: No class associated with this question"
      );
    ...
    if (!selectedLanguage || !question.languages.includes(selectedLanguage)) {
      console.log(
        "[QuestionSubmission] Custom input run blocked: Invalid or undefined language",
        selectedLanguage
      );
      setStatusMessage("Cannot run code: No valid language selected");
      return;
    ...
    if (!customInput.trim()) {
      console.log(
        "[QuestionSubmission] Custom input run blocked: No custom input provided"
      );
    ...
    if (!customInput.match(/^\[\s*-?\d+(\s*,\s*-?\d+)*\s*\]$/)) {
      console.log(
        "[QuestionSubmission] Custom input run blocked: Invalid array format"
      );
    ...
    if (expectedOutput && typeof expectedOutput !== "string") {
      console.log(
        "[QuestionSubmission] Custom input run blocked: Invalid expected output"
      );
    ...
    try {
      ...
      console.log(
        "[QuestionSubmission] Run code with custom input result:",
        result
      );
      setApiResponse({
      ...
      setStatusMessage(
        `Custom input run ${result.testResults.passed ? "passed" : "failed"}`
      );
    } catch (err) {
      console.error(
        "[QuestionSubmission] Run code with custom input error:",
        err
      );
      setApiResponse({
      ...
      setStatusMessage(
        `Custom input run failed: ${err.message || "Unknown error"}`
      );
    } finally {
  ...
  const handleLanguageChange = (e) => {
    ...
    if (
      ...
      console.log(
        "[QuestionSubmission] Starter code for",
        newLanguage,
        ":",
        starterCode?.code
      );
  ...
  const renderTestCaseResults = (results, source = "submission") => {
    console.log("[QuestionSubmission] Rendering test case results:", {
      results,
      source,
    });
    ...
    if (
      question?.type &&
      ["singleCorrectMcq", "multipleCorrectMcq", "fillInTheBlanks"].includes(
        question.type
      )
    ) {
      const isCorrect =
        apiResponse?.submission?.isCorrect ?? submission?.isCorrect ?? false;
      const explanation =
        apiResponse?.submission?.explanation ??
        submission?.explanation ??
        question.explanation ??
        "";
      const submittedAnswer =
        apiResponse?.submission?.output ?? submission?.output ?? "";
      const passedTestCases =
        apiResponse?.submission?.passedTestCases ??
        submission?.passedTestCases ??
        (isCorrect ? 1 : 0);
      const totalTestCases =
        apiResponse?.submission?.totalTestCases ??
        submission?.totalTestCases ??
        1;

      console.log(
        "[QuestionSubmission] Rendering non-coding question result:",
        {
          isCorrect,
          explanation,
          submittedAnswer,
      ...
      return (
        <div className="mt-4 p-4 rounded-xl border shadow-sm backdrop-blur-sm">
          ...
          <div
            className={`flex items-center p-4 rounded-lg border backdrop-blur-sm ${
              isCorrect
          ...
          >
            ...
            >
              {isCorrect ? (
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M10 18a8 0 100-16 8 0 000 16zm3.707-9.293a1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 0 00-1.414 1.414l2 2a1 0 001.414 0l4-4z"
                    clipRule="evenodd"
                  />
                </svg>
              ) : (
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M10 18a8 0 100-16 8 0 000 16zM8.707 7.293a1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                    clipRule="evenodd"
                  />
                </svg>
              )}
            ...
            <div className="ml-3">
              ...
              {(apiResponse?.submission?.score !== undefined ||
                submission?.score !== undefined) && (
                <p className="text-sm text-gray-600 mt-2">
                  Score:{" "}
                  {apiResponse?.submission?.score ?? submission?.score ?? 0}
                </p>
              )}
              {(apiResponse?.submission?.submittedAt ||
    ...
    if (!results) {
      ...
      return (
        <div className="mt-4 p-4 bg-yellow-50/80 backdrop-blur-sm rounded-xl shadow-sm border border-yellow-200">
          <p className="text-sm text-yellow-700 font-semibold">
            Invalid test case results format
          </p>
        </div>
      );
    }
        ...
        <div className="space-y-4">
          ...
          {source === "run" && (
            <div className="mt-4 p-4 bg-blue-50/80 backdrop-blur-sm rounded-xl border border-blue-200">
              ...
              <p className="text-sm text-blue-800 mt-2">
                Note: No score is assigned for running code.
              </p>
            </div>
          )}
        </div>
  ...
  console.log(
    "[QuestionSubmission] Rendering main component with question:",
    question.title
  );
  ...
  return (
    ...
    >
        ...
        <div className="border-b border-gray-200 bg-white px-6 py-3">
          <div className="grid gap-3 text-center text-sm sm:grid-cols-3">
            <div className="rounded-md bg-blue-50 px-3 py-2">
              ...
              <div className="text-lg font-bold text-blue-900">{formatDurationMs(totalTimerMs)}</div>
            </div>
            <div className="rounded-md bg-purple-50 px-3 py-2">
              ...
              <div className="text-lg font-bold text-purple-900">{formatSeconds(sectionTimerSeconds)}</div>
            </div>
            <div className={`rounded-md px-3 py-2 ${questionLocked ? 'bg-red-50' : 'bg-green-50'}`}>
              <div className={`text-xs font-semibold uppercase tracking-wide ${questionLocked ? 'text-red-700' : 'text-green-700'}`}>Question Remaining</div>
              <div className={`text-lg font-bold ${questionLocked ? 'text-red-900' : 'text-green-900'}`}>{formatSeconds(questionTimerSeconds)}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {examContext && questionLocked && (
        <div className="border-b border-red-200 bg-red-50 px-6 py-2 text-sm text-red-700 text-center">
          Time for this question has expired. You may review your work but cannot submit new changes.
        </div>
      )}
                    ...
                    <pre className="bg-gray-50 p-4 rounded-lg mt-2 overflow-x-auto text-sm border border-gray-100">
                      <code>
                        {question.starterCode.find(
                          (sc) => sc.language === selectedLanguage
                        )?.code || "No starter code available"}
                      </code>
                    </pre>
                  </>
                )}
        ...
        >
          <div className="flex-1 flex flex-col overflow-hidden p-4 relative">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              >
                ...
                {(question.type === "coding" ||
                  ...
                  !isFullscreen && (
                    <div
                      className="flex-shrink-0 border-t pt-2 space-y-2"
                ...
                {!isFullscreen && (
                  <div
                    className="flex-shrink-0 flex justify-end space-x-2 border-t pt-3 mt-2"
                    style={{ borderColor: "var(--card-border)" }}
                  >
                    {(question.type === "coding" ||
                      question.type === "fillInTheBlanksCoding") && (
                      <>
                        <button
                          ...
                          className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                        ...
                        <button
                          ...
                          className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 ${
            ...
            {(apiResponse || submission) && !isFullscreen && (
              <div
                className="overflow-y-auto border-t p-4 flex-shrink-0 max-h-[40vh]"
                style={{
              ...
              >
                {(apiResponse?.submission || submission) && (
                  <>
                    ...
                    >
                      <h3 className="text-lg font-semibold">
                        {apiResponse?.error
                          ? "Error Response"
                          : `${
                              apiResponse?.type === "submission"
                                ? "Submission"
                                : apiResponse?.type === "customRun"
                                ? "Custom Input Run"
                                : "Run"
                            } Response`}
                      </h3>
                      <p className="mt-2 text-sm">
                        {apiResponse?.message || "Action completed"}
                      </p>
                    </div>
                    ...
                    <div className="mt-4">
                      <div
                        className={`rounded-xl p-4 backdrop-blur-sm ${
                          apiResponse?.submission?.isCorrect ||
                    ...
                    {(apiResponse?.testResults ||
                      (apiResponse?.submission?.output &&
                        apiResponse?.type !== "submission")) &&
                      renderTestCaseResults(
                        apiResponse?.testResults ||
                          JSON.parse(apiResponse?.submission?.output || "[]"),
                        apiResponse?.type
                      )}
                  </>
                )}
                ...
                {runResults &&
                  ...
                  renderTestCaseResults(runResults, "run")}
              </div>
            )}
          </div>
        </div>
      ...
      {isFullscreen && (
        ...
        >
          {/* Fullscreen Header */}
          <div
            className="flex-shrink-0 border-b p-4 flex items-center justify-between"
            style={{
              backgroundColor: "var(--card-white)",
              borderColor: "var(--card-border)",
            }}
          >
            <div className="flex items-center gap-4">
              <div>
                ...
                >
                  Press{" "}
                  <kbd className="px-1.5 py-0.5 bg-gray-100 rounded border border-gray-300 text-xs">
                    ESC
                  </kbd>{" "}
                  or{" "}
                  <kbd className="px-1.5 py-0.5 bg-gray-100 rounded border border-gray-300 text-xs">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${
                  isRunning || isRunningCustom || !isQuestionActive
                    ? "bg-gray-400 cursor-not-allowed"
                    : "bg-blue-600 hover:bg-blue-700"
                }`}
              >
