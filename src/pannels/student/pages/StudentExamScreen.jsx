...
const StudentExamScreen = () => {
  ...
  // Debug logging
  useEffect(() => {
    console.log('[ExamScreen] Component mounted/updated', { examId, hasExam: !!exam, hasAttempt: !!attempt, loading, error });
  }, [examId, exam, attempt, loading, error]);
  ...
  const syncAttempt = useCallback(async () => {
    try {
      ...
      const updatedAttempt = attemptRes.data.attempt;
      setAttempt(updatedAttempt);
      
      // Initialize submitted questions from attempt answers
      if (updatedAttempt?.answers) {
        updatedAttempt.answers.forEach((ans) => {
          if (ans.submissionId) {
            submittedQuestionsRef.current.add(ans.questionId);
          }
        });
      }
  ...
  const initializeExam = useCallback(async () => {
    ...
    try {
      ...
      const remaining = new Date(initialAttempt.endsAt).getTime() - Date.now();
  ...
  const handleManualSubmit = useCallback(async () => {
    ...
    try {
      ...
      setAttempt(updated);
      exitFullscreen();
      // Navigate directly to exam list
      navigate('/student/exams');
    } catch (err) {
  ...
  }, [attempt, examId, exitFullscreen, navigate, syncAttempt]);
  ...
  const handleWorkspaceSubmit = useCallback(async (questionDetail) => {
    ...
    if (submittedQuestionsRef.current.has(questionDetail._id)) {
      setWorkspaceState((prev) => ({ 
        ...prev, 
        statusMessage: 'This question has already been submitted. You cannot submit again.' 
      }));
      return;
    }
    ...
    try {
      ...
      submittedQuestionsRef.current.add(questionDetail._id);
      
      // Show appropriate message based on question type
      const isCodingQuestion = ['coding', 'fillInTheBlanksCoding', 'codingWithDriver'].includes(questionDetail.type);
      if (isCodingQuestion) {
      ...
      {activeQuestion && questionMap.has(activeQuestion.questionId) && (
        <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
          <QuestionWorkspace
            ...
            isQuestionSubmitted={submittedQuestionsRef.current.has(activeQuestion.questionId)}
            ...
            isQuestionActive={!questionLocked && !isClosed && !submittedQuestionsRef.current.has(activeQuestion.questionId)}
